...
namespace Shopbanhang
{
    public partial class HoaDon : Form
    {
        ...
        {
            string str;
            str = "SELECT NgayBan FROM tblHoaDon WHERE MaHDBan = N'" + txtMaHDBan.Text + "'";
            txtNgayBan.Text = Functions.ConvertDateTime(Functions.GetFieldValues(str));
            str = "SELECT MaNhanVien FROM tblHoaDon WHERE MaHDBan = N'" + txtMaHDBan.Text + "'";
            cboMaNhanVien.Text = Functions.GetFieldValues(str);
            str = "SELECT MaKhach FROM tblHoaDon WHERE MaHDBan = N'" + txtMaHDBan.Text + "'";
            cboMaKhach.Text = Functions.GetFieldValues(str);
            str = "SELECT TongTien FROM tblHoaDon WHERE MaHDBan = N'" + txtMaHDBan.Text + "'";
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            float sl, SLcon;
            float tong, Tongmoi;
            sql = "SELECT MaHDBan FROM tblHoaDon WHERE MaHDBan=N'" + txtMaHDBan.Text + "'";
            if (!Functions.CheckKey(sql))
            {
                ...
                sql = "INSERT INTO tblHoaDon(MaHDBan, NgayBan, MaNhanVien, MaKhach, TongTien) VALUES (N'" + txtMaHDBan.Text.Trim() + "','" +
            ...
            sl = Convert.ToInt32(Functions.GetFieldValues("SELECT SoLuong FROM tblHang WHERE MaHang = N'" + cboMaHang.SelectedValue + "'"));
            ...
            Functions.RunSQL(sql);
            LoadDataGridView();
            // Cập nhật lại số lượng  mặt hàng vào bảng tblHang
            SLcon = sl - float.Parse(txtSoLuong.Text);
            sql = "UPDATE tblHang SET SoLuong =" + SLcon + " WHERE MaHang= N'" + cboMaHang.SelectedValue + "'";
            Functions.RunSQL(sql);
            // Cập nhật lại tổng tiền cho hóa đơn bán
            tong = float.Parse(Functions.GetFieldValues("SELECT TongTien FROM tblHoaDon WHERE MaHDBan = N'" + txtMaHDBan.Text + "'"));
            Tongmoi = tong + float.Parse(txtThanhTien.Text);
            sql = "UPDATE tblHoaDon SET TongTien =" + Tongmoi + " WHERE MaHDBan = N'" + txtMaHDBan.Text + "'";
            Functions.RunSQL(sql);
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Bạn có chắc chắn muốn xóa không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                //Xóa hóa đơn
                sql = "DELETE tblHoaDon WHERE MaHDBan=N'" + txtMaHDBan.Text + "'";
                Functions.RunSqlDel(sql);
                ResetValues();
                LoadDataGridView();
                btnXoa.Enabled = false;
        ...
        private void btnInHoaDon_Click(object sender, EventArgs e)
        {
            ...
            exRange.Range["A1:B1"].Value = "Cửa hàng gia dụng YoKo";
            exRange.Range["A2:B2"].MergeCells = true;
            exRange.Range["A2:B2"].HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
            exRange.Range["A2:B2"].Value = "116 Lương Đình Của - Kim Liên - Hà Nội";
            ...
            sql = "SELECT a.MaHDBan, a.NgayBan, a.TongTien, b.TenKhach, b.DiaChi, b.DienThoai, c.TenNhanVien FROM tblHoaDon AS a, tblKhachHang AS b, tblNhanVien AS c WHERE a.MaHDBan = N'" + txtMaHDBan.Text + "' AND a.MaKhach = b.MaKhach AND a.MaNhanVien = c.MaNhanVien";
